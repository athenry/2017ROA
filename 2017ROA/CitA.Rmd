...
## check "other"s
```{r}
Other <- AlbertaData[which(AlbertaData$Dept == "Other"),]
View(Other)
```
## Report number of articles per year per department 
```{r}
RepData <- subset(AlbertaData, Dept != "Other", select = c(UT, TC, PY, Dept))
ppy <- ggplot(unique(RepData), aes(PY)) + geom_bar(aes(fill=Dept))
```
## Report number of citations per department per year
cpy <- ggplot(unique(RepData), aes(Dept, TC)) + geom_col(aes(fill=PY), position = position_stack(reverse = TRUE))