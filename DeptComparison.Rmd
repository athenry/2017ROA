...
```
Publication data was grouped by department to facilitate comparison between appopriate groups. Of course, the disciplines are grouped differently at different schools, and not all departments have an Alberta-equivalent. 
...
```
Number of faculty members were collected for each school, in order to allow for a more representative comparison.

```{r read in faculty data}
facAlberta <- read.csv("https://docs.google.com/spreadsheets/d/e/2PACX-1vT04KVZbmDhkrE43WSPjYgx2kKEgUdApH_z4UawFtmDKhu4sYhOaiMdZH0fdo3zo6rSSimAg7HXa7xc/pub?gid=1250905174&single=true&output=csv")

facCalgary <- read.csv("")
```
Mean number of citations per year per school
